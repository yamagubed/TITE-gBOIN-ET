 ...
 for(i in 1:nesc){
  ...
  tite.df <- rbind(tite.df,
             data.frame(dose   = curdose,
                        ...
                        nets   = nETS,
                        endeff = t.enter+apply(as.matrix(1:ncoh),1,function(x){min(time.eff[x],tau.E[s2])}),
                        nees   = nEES))
  ...
  for(ds in 1:lds){
   if(sum(tite.df$dose==ds)>0){
    ...
    n.nEES <- x.nEES+sum(1-compsub.E$nees)+sum(t.decision-pendsub.E$enter)/tau.E[s2]
...
write.csv(out.df,"../../data/simulation1_obd_TITE-gBOIN-ET.csv")
...
write.csv(out.df,"../../data/simulation1_dose_allocation_TITE-gBOIN-ET.csv")
