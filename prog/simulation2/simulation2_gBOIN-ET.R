...
for(s3 in 1:ls3){
 ...
 for(i in 1:nesc){
  ...
  for(j in 1:ncoh){
   ...
   for(k in 1:maxev){
    if(k==1){
     time.tox <- rweibull(1,shape=zetta.T1,scale=zetta.T2)
    }else{
     time.tox <- time.tox+rweibull(1,shape=zetta.T1,scale=zetta.T2)
    }
    event.T  <- as.numeric(time.tox<=tau.T)
    wk.grade <- c(wk.grade,event.T*((1:3)%*%rmultinom(1,1,efftoxp$toxp[2:4,dlab]))[1,1]+1)
    if(event.T==0){
     grade <- c(grade,max(wk.grade))
     break
    }
   }
   ...
   for(k in 1:maxev){
    if(k==1){
     time.eff <- rweibull(1,shape=zetta.E1,scale=zetta.E2)
    }else{
     time.eff <- time.eff+rweibull(1,shape=zetta.E1,scale=zetta.E2)
    }
    event.E     <- as.numeric(time.eff<=tau.E[s2])
    wk.response <- c(wk.response,event.E*((1:3)%*%rmultinom(1,1,efftoxp$effp[2:4,dlab]))[1,1]+1)
    if(event.E==0){
     response <- c(response,max(wk.response))
     break
    }
   }
  ...
  ETS  <- apply(as.matrix(grade),1,function(x){return(weight.T[x])})
  nETS <- ETS/max(weight.T)
  EES  <- apply(as.matrix(response),1,function(x){return(weight.E[x])})
...
write.csv(out.df,"../../data/simulation2_obd_gBOIN-ET.csv")
...
write.csv(out.df,"../../data/simulation2_dose_allocation_gBOIN-ET.csv")
